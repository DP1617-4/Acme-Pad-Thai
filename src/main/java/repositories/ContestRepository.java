...
public interface ContestRepository extends JpaRepository<Contest, Integer>{
	...
	@Query("select c.qualified from Contest c join left join c.qualified q left join q.scores s where c.id = ?1 order by sum(case when s.likes = 1 then 1 else 0 end) DESC ")
